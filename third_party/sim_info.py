...
class SimInfo:
    ...
    def close(self):
        for m in (getattr(self, '_acpmf_physics', None),
                  getattr(self, '_acpmf_graphics', None),
                  getattr(self, '_acpmf_static', None)):
            if m is None:
                continue
            try:
                m.close()
            except BufferError:
                # Some ctypes exported pointers may still exist; ignore on shutdown
                pass
            except Exception:
                # Be defensive: ignore other close-time errors
                pass
